...
1. `POP-002` — add the universal Explorer per-record narrative and typed-source contract.
2. `POP-003` — migrate the six reviewed families into the full relationship and resource plane.
3. `POP-004` — implement shared authority, geography, regulator, redress and link-receipt infrastructure.
4. `PACK-001` through `PACK-008` — populate the eight contracted domain packs.
5. `ASSURE-001` — reconcile search, graph, provenance, source links and local browser journeys and freeze a local population candidate.
