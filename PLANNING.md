...
branch protection is now active after the owner separately authorised public
...
`okf.publication.json` now makes this lockstep boundary machine-readable. The
local checker requires both declared documentation and `CHANGELOG.md` for a
controlled change, including dependency and workflow changes. The shared
Explorer profile validates the complete cross-repository contract; the local
checker protects this repository's manual-only, exact-byte policy without
introducing routine hosted CI.
...
protected-main commit and pass the real-browser verification gate. Rendered
documents use the existing browser-safe HTML notice for their MIT and
attribution boundary; the owner approved a bounded correction on 17 August
2026 after Edge blocked the first deployment's extensionless `LICENSE` route.
...
verified. That bounded sample proposal was superseded by the later authorised
population-complete preview.

The owner separately authorised exhaustive link-only reference-family
...
not change the sample release gates or authorise snapshots, broad leaf
...
high-impact operational claims. The owner has separately authorised the frozen
...
The authorised publication scope is the population-complete educational
...
Before publication authorisation, pull requests used this statement:

> Publication status: local validation only. GitHub Pages was not updated.
> Publication requires an explicit owner request.

This authorised publication pull request instead records the owner request and
