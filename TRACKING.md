...
| DOC-PUB-001 | Add a governed static-document publication fast path and publish the PRD | Ready for exact-head deployment | Explicit frontmatter discovers nominated documents, safe deterministic HTML and a generated document index and manifest. The dependency-graph gate rejects corpus, semantic, schema and workflow changes. The PRD is rendered as draft review material; merge does not update Pages and the owner-requested manual exact-head deployment plus browser verification remain required. |
