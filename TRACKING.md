...
| PAGES-002 | Deploy the rich-runtime correction | Complete | Manual workflow run `31737084740` deployed the 1,814-file frozen base plus the authorised Explore overlay from exact main merge `d1efb6b...` |
| VERIFY-002 | Verify the rich-runtime publication | Complete | The live base descriptor and large manifest matched their frozen SHA-256 identities; the full Explorer loaded all 293 families and completed the missed-rubbish search-to-source journey with no console errors |
...
| DOC-PUB-001 | Add a governed static-document publication fast path and publish the PRD | Complete: correction ready for exact-head deployment | Explicit frontmatter discovers nominated documents, safe deterministic HTML and a generated document index and manifest. The first deployment exposed an Edge block on the extensionless `LICENSE` route; the owner-approved bounded correction links every footer to the existing browser-safe licence and attribution notice. Corpus and semantic bytes remain unchanged. Merge does not update Pages; the authorised exact-head redeployment and browser verification remain external gates. |
