...
| RIGHTS-001 | Resolve repository and bounded source-family licensing | Complete | Merged in PR #6 with 16 slice hosts; later inventory, authority and Pack 3 decisions extend dated evidence to 29 hosts while retaining link-and-summary use and zero snapshots |
...
| INVENTORY-001 | Exhaustive external reference-family inventory | Complete | Owner authorised link-only discovery on 2026-08-07; 142 external reference records, 120 assessed domain/jurisdiction cells, 25 source-host rights decisions, 12 tracked gaps and zero snapshots |
| CORPUS-001 | Approve the staged 250–400 service-family denominator | Complete | Owner approved [`service-family-denominator.v1`](source/service-family-denominator.v1.yaml) on 2026-08-07: 293 named normalised families in three staged waves |
...
| ASSURE-001 | Freeze a local population-complete candidate | Complete: local pass | [Population assurance](evaluation/reviews/population-assurance-2026-08-08.md) reconciles 293 dossiers, 24 domains, 48 processes, 104 questions, 879 links and 15,810 relationships with zero blocking omissions; exact artefacts are hashed and publication remains disabled |
| SCOPE-001 | Approve publication claims and limitations | Complete | Owner authorised the full population-complete corpus as an educational preview; not release-grade, official advice or formal CPSV-AP/Open Referral/OWL/SHACL conformance |
...
| PUB-001 | Authorise publication of the frozen candidate | Complete | Owner explicitly authorised GitHub Pages publication and OKF Explorer evaluation on 2026-08-08 |
| PAGES-001 | Deploy frozen candidate bytes to GitHub Pages | Complete | Owner authorised public repository visibility; workflow run `31297841419` and deployment `5815993749` promoted the pinned 1,549-file unit successfully |
...
| EXPLORE-001 | Build the Explore OKF reference and review surface | Complete: strict local pass | A manifest-pinned standalone interface, machine sidecar, review hub, curated learning library and provider-neutral AI prompt route preserve descriptor SHA-256 `ff69f016…` and every corpus/runtime byte. The overlay may replace only the frozen root landing page after matching its old SHA-256; every other target is additive or preserved. Deterministic projection, overlay, documentation, schema, security and unit checks cover 293 families, explicit ordinary/exception order, composite-jurisdiction routing, provenance, no active network or storage and safe rendering |
| AI-EVAL-001 | Establish isolated AI-consumer evaluation and run a Codex development preflight | Complete: local harness pass, not a conformance pass | Answer-blind local runs diagnose prompt, jurisdiction and decision-boundary ambiguities. Evaluator inputs are not yet receipt-bound, so no behavioural or comparative result is published; raw model prose remains ignored, independent review and the complete matrix remain open, and promotion eligibility is false |
| AI-EVAL-002 | Complete the two-model, two-condition 104-question comparison | Closed: incomplete private diagnostic attempts | Two no-retry attempts stopped before completion and independent review remains pending. Current evaluator inputs are not yet bound to their immutable runner receipts, so no response counts, scores or model comparison are published as evidence. All packs are now development-calibration material and promotion eligibility is false |
...
| EXPLORE-PAGES-001 | Authorise and deploy the Explore OKF review overlay | Active: owner-authorised, deployment pending | The owner authorised public review on 13 August 2026. A manual dispatch will package the frozen 1,814-file base, one hash-bound root landing replacement and the additive Explore and learning files. It requires the exact protected-main merge plus `EXPLORATORY-NOT-RELEASE-GRADE`; no dispatch or Pages update has yet occurred |
| EXPLORE-VERIFY-001 | Verify the exact deployed Explore OKF publication | Pending deployment | Match the deployed descriptor, projection and overlay hashes and pass real-browser identity, alias-search, jurisdiction, ordinary/exception, provenance and official-source journeys before any Explore OKF URL is labelled verified |

## Current operating state

- GitHub remote: public repository under `chris-page-gov/okf-uk-living` after
  explicit owner authorisation on 2026-08-09.
...
- Explore OKF: the owner-authorised review overlay passes local deterministic,
  documentation, security and publication validation. It preserves the exact
  Claude-tested descriptor and all corpus/runtime bytes, replaces only the
  hash-bound root landing target and adds the Explore and learning routes. It
  has not yet been deployed or verified publicly; do not describe an Explore
  OKF URL as verified.
- Acquisition: 89 exhaustive-inventory references, 53 implemented-slice
  ...
  293 families is authorised; unbounded or unstaged acquisition stays disabled.
...
- Review state: the integrated local Explorer remediation rerun passed on
  ...
  release-grade candidate. Owner authorisation, exact deployment and public
...
- AI-consumer evaluation: two private no-retry comparison attempts were
  preserved but remained incomplete. All 104 questions are now
  development-calibration material. No response counts, model score or
  comparison are published until evaluator inputs are receipt-bound and
  independent review is complete.
- Reference readiness: all 120 declared domain/reference-jurisdiction cells are
  assessed (96 national covered, 24 local discovery/archetype cells). The six
  requested owner decisions are complete and every tracked gap has a
  population-gate disposition; named specialist reviewer acceptance remains a
  release-grade external-review gate.
- Population implementation: owner-authorised on 2026-08-08 in eight packs;
  foundation, universal Explorer contract, six-family migration, shared
  authority/source infrastructure, `PACK-001` through `PACK-008` and
  `ASSURE-001` complete. No population delivery item remains active.
