...
| Closed infrastructure | `GAP-LOCAL-TOPOLOGY` | The 2026-08-08 registry implements 382 principal areas, 19 strategic authorities and source-native fallbacks; family-specific functional bodies remain pack work. | Apply the shared identities and cite current leaf responsibility in each local family. |
...
| Pack implementation | `GAP-PRIVATE-DEPENDENCIES` | The reusable ten-sector regulator/register and redress map is implemented; family-specific providers and selection bases remain. | Apply regulator-first discovery and record the selection basis in each affected dossier. |
...
| Pack implementation | `GAP-SECTOR-REDRESS` | Shared identities and the five-level taxonomy are implemented; service-family sequences remain incomplete. | Cite the actual provider, statutory, tribunal, regulator/ombudsman and court sequence for each family. |
| Closed infrastructure | `GAP-LGSL-CURRENCY` | The current ESD Services-list location was re-observed on 2026-08-08. | Use identifiers as optional mappings only; never as evidence of current availability. |
| Pack implementation | `GAP-FRESHNESS-AUTOMATION` | Metadata-only receipt tooling is implemented, but devolved portals do not expose one shared update API. | Set per-family review intervals and run reviewed live audits outside offline checks. |
| Closed policy | `GAP-BILINGUAL-IDENTITY` | Explicit publisher pairing is required; labels, translated slugs and parallel URLs are prohibited as identity evidence. | Apply and evidence the rule in each bilingual family variant. |
...
The shared-authority/source-infrastructure phase closed local topology,
LGSL-currency and bilingual-identity policy on 2026-08-08, and supplied reusable
private-dependency, sector-redress and link-receipt infrastructure. The remaining
ledger is family-pack implementation and specialist-review work. It does not
